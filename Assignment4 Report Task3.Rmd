...
## Compare two methos that were ultilized in Task Two and Task Three.
```{r}
...
```
Due to these are two different methods, so it is not easy and reasonable to compare them directly. Therefore, I limited the range of these sentiment words, just similar what I did in the previous diagram. After defining the scale, I made a bar plot to explain the result. In each session, the sentiment trends in vocabulary are roughly the same, but the specific values are different. However, I think setimentr method is better the Bing.
